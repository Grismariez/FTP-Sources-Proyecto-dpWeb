...
if(isset($_GET['opc']))
{
	...
	if($opc=='Buscar')
	{
		...
		$sql="Select * from detallecompra where producto_detallec like '%".$frase."%'";
	}
	if($opc=='Detalle')
	{
		$id=$_GET['id_detallec'];
		$sql="Select * from detallecompra where id_detallec=$id";
	}
	if($opc=='Eliminar')
	{
		$id=$_GET['id_detallec'];
		$sql="Delete from detallecompra where id_detallec=$id";
		header("location:admindetalle.php");
	}
	if($opc=='Modificar')
	{
		...
		$sql2="Select * from detallecompra where id_detallec=$id";
		$mod=mysql_query($sql2);
		$usuario=mysql_fetch_array($mod);
?>	

		<form name="frmMod" action="admindetalle.php?opc=Actualizar" method="POST">
		<input type="hidden" name="tmpId" value="<?php echo $usuario['id_detallec']; ?>">
...
<?php
	...
	if($opc=='Actualizar')
	{
		...
		$sql="update detallecompra set usuario_mje='$nom', email_mje='$correo', asunto_mje='$asunto' where id_mje=$id";
		header("location:admindetalle.php");
	}
}
$result=mysql_query($sql);
?>
<form name="frm" method="POST" action="admindetalle.php?opc=Buscar">
<table border="0" cellpadding="0" cellspacing="3px">
<tr>
<th class="tha" colspan="7"><h1>Mantenimiento Detalle de Compra</h1></th>
</tr>

<tr>
<td class="tda" width="69"></td>
...
<td class="tda" colspan="4">
	...
	<a class="aa" href="admindetalle.php">Ver Todos</a>
</td>
</tr>
...
<th class="tha">Precio</th>
<th class="tha">Cantidad</th>
<th class="tha" colspan="2" ></th>
</tr>
<?php
while($rows=mysql_fetch_array($result)){ 
?>
<tr>
<td class="tda"><?php echo $rows['id_detallec']; ?></td>
<td class="tda"><?php echo $rows['producto_detallec']; ?></td>
<td class="tda"><?php echo $rows['precio_detallec']; ?></td>
<td class="tda"><?php echo $rows['cantidad_detallec']; ?></td>
<td class="tda">&nbsp;
<a class="aa" href="admindetalle.php?opc=Detalle&id_detallec=<?php echo $rows['id_detallec']; ?>">
Detalle</a>&nbsp;</td>
<td width="70" class="tda">&nbsp;
<a class="aa" href="admindetalle.php?opc=Eliminar&id_detallec=<?php echo $rows['id_detallec']; ?>"
...
?>
<br><br>
<a class="aa" href="Detalle_admin.php" style="background-color:#9FF">Regresar</a>  
   </center> 
