...
$alias=$_SESSION['miusuario'];
$n = mysql_fetch_array(mysql_query("SELECT sexo_reg As 'Sexo' FROM registrados WHERE nickname_reg = '$alias'"));
$sexo=$n['Sexo'];
$p = mysql_fetch_array(mysql_query("SELECT pais_reg As 'Pais' FROM registrados WHERE nickname_reg = '$alias'"));
$pais=$p['Pais'];
...
'$preg13','$detalle13','$preg14','$preg15','C')";

// Instrucción para evitar que al refrescar página vuelva a enviarse respuestas de encuesta por mismo usuario
$sql2 = "SELECT COUNT(id_encu) As 'Encuesta enviada' FROM (SELECT id_encu FROM encuesta WHERE alias_encu = '$alias' And
sexo_encu = '$sexo' And pais_encu = $codPais And answerpreg1_encu = '$preg1' And answerpreg2_encu = '$preg2')a";

// Ejecutando filtro para revocar mensaje repetido
$countEncu = mysql_query($sql2); 
$fila = mysql_fetch_array($countEncu); 

if (($alias != null) or ($codPais != null))
{	
	if ($fila['Encuesta enviada'] == 0)
	{
		...
		if($result)
		{
			header('location:../Encuesta_cliente.php');
			session_start();
			$_SESSION['alertaEncuC'] = "El envío de su encuesta ha sido satisfactorio!!";
		}
		else
		{
			header('location:../Encuesta_cliente.php');
			session_start();
			$_SESSION['alertaEncuC'] = "Proceso Fallido. Intentelo de nuevo más tarde!!";
		}
	}
	else
	{
		header('location:../Encuesta_cliente.php');
		session_start();
		$_SESSION['alertaEncuC'] = "Lo sentimos, hemos encontrado en nuestros registros una encuesta parecida a esta. Favor envíe una diferente!!";
	}
}
mysql_close();
